...
操作:
    ...
    シークバー:
        マウスで移動すると、指定フレームをMCAPから再読込みして表示する
...
def build_depth_frame_index(
    mcap_files: Sequence[Path],
    topic: str,
) -> List[DepthFrameIndex]:
    """
    対象Depthトピックの軽量timestamp indexを作成する。
    ...
    Returns:
        DepthFrameIndexの時系列リスト。
    """
    frame_index: List[DepthFrameIndex] = []

    for mcap_path in mcap_files:
        print(f"[INDEX] {mcap_path}")
...
def frame_index_timestamps(
    frame_index: Sequence[DepthFrameIndex],
) -> List[int]:
    """
    DepthFrameIndexからtimestampだけの配列を作る。
...
def read_depth_message_at(
    frame_entry: DepthFrameIndex,
    topic: str,
    reader_cache: DepthFrameReaderCache,
) -> Tuple[int, object]:
    """
    timestamp indexで指定されたDepthメッセージをMCAPから1枚だけ読み込む。
    ...
    raise RuntimeError(
        "Could not load indexed depth frame: "
...
def run_playback(
    ...
    """
    timestamp indexを使ってDepth画像を再生・シーク・保存する。

    通常再生:
        current_positionの索引から1枚を読み、
        表示後にcurrent_positionを1増やす。

    シーク:
        trackbar callbackがrequested_positionを設定する。
        再生ループはその位置へcurrent_positionを変更し、
        対応するMCAPファイルを再オープンして指定timestampを読む。

    メモリ:
        RAMへ常駐するのはtimestampとMCAPパスの索引だけで、
        Depth画像は現在表示中の1枚だけである。

    動画・PNG保存:
        シーク後も、ユーザーが実際に表示した順番で保存する。
        したがって後戻り、重複、飛び越しも出力へ反映される。
    """
    ...
    def on_seek_trackbar(position: int) -> None:
        ...
        再生ループ自身によるsetTrackbarPos()では要求を作らず、
        ユーザー操作の場合だけrequested_positionへ記録する。
        """
    ...
    def update_trackbar(position: int) -> None:
        """
        現在の再生位置をtrackbarへ反映する。

        setTrackbarPos()でもcallbackが発火し得るため、
        internal_updateフラグでユーザーシークとの混同を防ぐ。
        """
        if args.no_display:
            return
    ...
    try:
        if not args.no_display:
            ...
            # OpenCVの実装によっては最大値0のtrackbarを作れないため、
            # 1フレームだけの場合もUI上の最大値は1にする。
            # callback側・シーク処理側で実際の範囲へclampする。
            trackbar_max = max(
                1,
            ...
            # createTrackbar()直後にcallbackが呼ばれる環境があるため、
            # 初期シーク要求を破棄する。
            seek_state.requested_position = None

        while current_position < len(selected_frames):
            update_trackbar(
                current_position
            )

            frame_entry = selected_frames[
                current_position
            ]

            timestamp_ns, msg = read_depth_message_at(
            ...
            while not frame_finished:
                ...
                relative_time_sec = (
                    timestamp_ns - full_bag_start_ns
                ) * 1.0e-9
                ...
                if command == "seek":
                    ...
                    if requested is not None:
                        ...
                        # シーク直後に、シーク前後の大きなtimestamp差だけ
                        # 待たされないよう、次フレームの待機基準をリセットする。
                        ...
                        print(
                            ...
                            f"time="
                            f"{(selected_frames[current_position].timestamp_ns - full_bag_start_ns) * 1.0e-9:.3f} s"
                        )
                ...
                if command == "step":
                    # pausedはTrueのまま、次フレームへ1枚だけ進む。
                    frame_finished = True
                    break

                # command=None:
                # 通常再生、またはSpaceで一時停止を解除した状態。
                frame_finished = True
            ...
            previous_displayed_timestamp_ns = timestamp_ns
            processed_count += 1
            current_position += 1

            if processed_count % 100 == 0:
                print(
                    f"[PROGRESS] {processed_count} displayed frames"
                )
    ...
    print("=" * 72)
    print(f"Indexed frames   : {len(frame_index)}")
...
def main() -> int:
    ...
    処理順:
        1. 引数を検証する
        2. MCAPファイル一覧を取得する
        3. timestampとMCAPファイルの軽量索引を作る
        4. 対象トピックの時刻とFPSを確認する
        5. Depth画像をランダムアクセスで再生・シーク・保存する
    """
    args = parse_args()
    ...
    try:
        ...
        frame_index = build_depth_frame_index(
